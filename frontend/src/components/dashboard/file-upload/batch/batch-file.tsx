...
export const BatchFile: React.FC<BatchFileProps> = ({ file }) => {
  ...
  const {
    ...
    fetchData: onCreateBatchExperiments,
  } = useApi({
  ...
  const { loading: loadingOnStartBatch, fetchData: onStartBatchExperiments } =
    useApi({
  ...
  const handleStartExperiment = () => {
    if (batchId) onStartBatchExperiments();
  };
  ...
  return (
    <div>
      ...
      <div className='flex justify-end gap-2 my-4 w-full'>
        ...
        <Button
          disabled={!isFileCreated || loadingOnStartBatch}
