...
import {
  prepareDatasetForMetrics,
  prepareDatasetForProbability,
} from './helpers';
import { MetricsChart } from './metricsChart';
import { Button } from '@/components/ui/button';
import { ChartBar, Table } from 'lucide-react';
import {
  analysisMetricsColumns,
  analysisProbabilityColumns,
} from '@/components/tables/table-columns/analysis-columns';
...
export const AnalysisModule = ({ experimentId }: { experimentId: string }) => {
  ...
  useEffect(() => {
    ...
    if (analysisData && analysisData.probability) {
      setProbabilityData(
        prepareDatasetForProbability(analysisData.probability)
      );
    }
  }, [analysisData]);
  ...
  return (
    <div>
      ...
      {viewMode === 'table' ? (
        <DynamicTable
          filterColumnKey={'service'}
          data={metricsData}
          columns={analysisMetricsColumns}
        />
      ) : (
      ...
      <DynamicTable
        ...
        columns={analysisProbabilityColumns}
      />
    </div>
  );
};
